...
*Last updated: June 28, 2026 (Daily gem cap — earning-only, never blocks gameplay)*
...
### Gameplay & Monetization (June 28, 2026)
- [x] **Daily gem cap is earning-only — never blocks gameplay** (PR #16, branch `MaloneySandboxAI/kinshasa`, not yet merged). The 100-gem/day cap previously set `soloBlocked` and locked free users out of solo play until midnight — a hard paywall on web and a complete dead-end on iOS (upgrade UI is hidden per Guideline 3.1.1). Now the cap limits earning only (still enforced via `recordGemsEarned` + `Math.min(totalEarned, dailyGemsRemaining)`); users always get to play and earn streak credit. `soloBlocked` now only handles hearts/round-limit gating (refillable with gems). Capped users see a positive "you've earned today's max gems" notice on the results screen (new `gemsCapped` prop on `ResultsScreen`) and, on iOS, a friendly "you've maxed out" toast instead of an upgrade upsell; web/Android keep the existing upgrade toast. Files: `app/play/page.tsx`, `components/rally/results-screen.tsx`, `CLAUDE.md`. Side benefit: also removes the same dead-end on Android.
- Note: this softens the strongest web paywall moment. To convert the freed-up players, the real next lever is **gem sinks** — today gems only buy heart refills, streak freeze, and stats deep-dive (`lib/hearts.ts`, `lib/gem-context.tsx`). More compelling sinks (cosmetics, power-ups, store) would make the cap a felt constraint and drive "upgrade for unlimited gems." See "Store page for gem spending" under Medium-term.
