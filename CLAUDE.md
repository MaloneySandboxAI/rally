...
**Hearts are localStorage-only — never server-synced (the "never decrement / reset between rounds" fix, June 28 2026).** `lib/sync.ts` is **live** despite its "localStorage-only" header comment: `syncToServer()` fires on every gem change (`lib/gem-context.tsx`) and `syncFromServer()` ("server wins — overwrite local") runs in `auth-gate.tsx` **on every navigation** (effect deps `[pathname, router]`). It used to round-trip hearts/rounds through the `user_state` table. Two compounding problems made hearts appear to never decrement (#38) and reset between rounds (#42): (1) `syncToServer`'s upsert payload included `subtopic_levels` and `diagnostic` columns that **do not exist** in `user_state` (see migration 006) — so **every server write failed silently** (caught by the try/catch), leaving the server's hearts value frozen; and (2) `syncFromServer` then restored that frozen value over the local decrement on the next navigation. (An earlier attempt that merely reordered the decrement before the gem award did not help, because the write never lands.) **Fix: hearts/rounds (`hearts`, `hearts_date`, `rounds_today`, `rounds_date`) were removed from BOTH `syncToServer` and `syncFromServer`.** Hearts now live purely in localStorage, managed by `lib/hearts.ts` with its daily reset — which is correct, since hearts reset daily and have no real cross-device value. **Don't re-add hearts/rounds to the sync layer** without first building a real conflict-resolution strategy (v1.1). Note: the phantom `subtopic_levels`/`diagnostic` columns are still in the `syncToServer` payload, so gem/streak/stats sync may also be silently failing — left untouched here to avoid a one-time "server-wins" clobber regression on gems; verify the live `user_state` schema before "fixing" it.
