...
**Hearts ↔ server-sync interaction (the "never decrement / reset between rounds" bug, fixed June 28 2026).** Despite its header comment claiming "localStorage-only," `lib/sync.ts` is **live**: `syncToServer()` fires on every gem change (`lib/gem-context.tsx`) and `syncFromServer()` ("server wins — overwrite local") runs in `auth-gate.tsx` **on every navigation** (its effect deps are `[pathname, router]`), and the `user_state` table exists in prod. `syncToServer` snapshots hearts straight from localStorage at call time. The award effect used to call `addGems()`/`markRoundCompleted()` (both trigger `syncToServer`) **before** `loseHeart()`, so the server received the **stale pre-decrement** count (5); the local decrement to localStorage was never the value that reached the server, and the next `syncFromServer()` on navigation restored 5 — so hearts appeared to never decrement (#38) and reset between rounds (#42). **Fix: the hearts/rounds decrement block now runs BEFORE the gem award**, so every `syncToServer` reads the already-decremented value and the server holds the correct count. **Don't move the `incrementRoundsToday()` + `loseHeart()` + `setHearts(getHearts())` block back below `addGems`/`markRoundCompleted`.** Residual minor edge (acceptable for v1.0.x, revisit with the v1.1 server-sync work): at a day boundary `syncFromServer` can momentarily restore yesterday's count, but the next `getHearts()` call (header/play mount) re-applies the daily reset and self-heals.
