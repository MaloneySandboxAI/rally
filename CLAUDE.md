...
**Hearts are localStorage-only — never server-synced (the "never decrement / reset between rounds" fix, June 28 2026).** `lib/sync.ts` is **live** despite its "localStorage-only" header comment: `syncToServer()` fires on every gem change (`lib/gem-context.tsx`) and `syncFromServer()` ("server wins — overwrite local") runs in `auth-gate.tsx` **on every navigation** (effect deps `[pathname, router]`). It used to round-trip hearts/rounds through the `user_state` table. Two compounding problems made hearts appear to never decrement (#38) and reset between rounds (#42): (1) `syncToServer`'s upsert payload included `subtopic_levels` and `diagnostic` columns that **do not exist** in `user_state` (see migration 006) — so **every server write failed silently** (caught by the try/catch), leaving the server's hearts value frozen; and (2) `syncFromServer` then restored that frozen value over the local decrement on the next navigation. (An earlier attempt that merely reordered the decrement before the gem award did not help, because the write never lands.) **Fix: hearts/rounds were first removed from sync; then (see "State sync is disabled" below) the entire server round-trip was turned off.** Hearts live purely in localStorage, managed by `lib/hearts.ts` with its daily reset.

### State sync is disabled — localStorage is the single source of truth (June 28 2026)
`lib/sync.ts`'s `syncToServer()` and `syncFromServer()` are now **no-ops**. The same broken round-trip that hid the hearts bug was also **resetting gems to 300 and streaks to 0**: `syncToServer`'s upsert wrote `subtopic_levels` and `diagnostic` columns that **don't exist** in `user_state` (migration 006), so every write failed silently and the server row stayed frozen at its **defaults** (`gems = 300`, `streak = 0`, …). `syncFromServer` ("server wins") runs in `auth-gate.tsx` on **every navigation** (effect deps `[pathname, router]`), so it restored those defaults over good local data — e.g. buy hearts until gems run low, press Home, gems snap back to 300. **Why not just fix the columns:** the first load after a fix would read the frozen 300 and wipe every existing user's real gem balance once. So both functions were disabled instead; `lib/gem-context.tsx` and `lib/hearts.ts` already persist to localStorage on their own, so nothing is lost. `getUserId`/`createClient`/`KEYS` are kept (unused for now) for the v1.1 rebuild. **v1.1:** build real server sync with a proper schema + merge strategy (don't blindly "server wins"), then re-enable. Don't re-enable these functions before then.
